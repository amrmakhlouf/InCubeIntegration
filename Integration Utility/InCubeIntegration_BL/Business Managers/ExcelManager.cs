...
namespace InCubeIntegration_BL
{
    public class ExcelManager : System.IDisposable
    {
        ...
        {
            return GetSonicDataTable(ResultsQueries[SheetNo], ref dtResults);
        }
        ...
        public Result SaveImportType(string ImportTypeName, string Description, DataTable dtSheets, List<string> Procs, DataTable dtColumns, SaveMode saveMode, ref string QueryString)
        {
            try
            {
...
INSERT INTO Int_ExcelImportTypes (ImportTypeID,Name,Description) VALUES (@ImportTypeID,'{0}','{1}');

--Int_ExcelImportSheets", ImportTypeName.Replace("'", "''"), Description.Replace("'", "''")));
...
IF NOT EXISTS (SELECT * FROM Int_Privileges WHERE PrivilegeID = @ImportTypeID AND PrivilegeType = 3)
	INSERT INTO Int_Privileges (PrivilegeID,PrivilegeType,ParentID) VALUES (@ImportTypeID,3,11);");
        ...
        public Result GetDataTypes(bool GetAll, int FilteredTypeID, ref DataTable dtTypes, ref DataTable dtSheets, ref DataTable dtColumns, ref DataTable dtProcedures)
        {
            try
            {
                ...
                if (FilteredTypeID != -1)
                    ...
                    qry += string.Format(" INNER JOIN Int_UserPrivileges UP ON UP.PrivilegeID = T.ImportTypeID AND UP.PrivilegeType = 3 AND UserID = {0} ORDER BY UP.Sequence", CoreGeneral.Common.CurrentSession.UserID);
        ...
        public Result ExecuteStoredProcedure(string ProcedureName)
        {
            try
            {
                incubeQry = new InCubeQuery(db_Sonic, ProcedureName);
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (DataRow dr in ColumnsList)
                    {
                        ...
                        {
                            dtCSVContetns.Columns.Add(ColumnName);
        ...
        public Result DumpDataToStagingTable(int SheetNo, string ExcelSheet, string StagingTable, DataRow[] Columns)
        {
            if (CreateTable(StagingTable, Columns) == Result.Success)
            {
                if (FileExt != FileExtension.csv)
        ...
        public Result CreateTable(string TableName, DataRow[] Columns)
        {
            try
            {
                string columns = "\r\n        [ID] INT IDENTITY(1,1),";
...
')", TableName, columns.Substring(0, columns.Length - 1));
        ...
        public Result InitializeDataReader(string QueryString)
        {
            try
            {
                if (excelConn.State != ConnectionState.Open)
                ...
                XlsDataReader = cmd.ExecuteReader();
                return Result.Success;
            }
            catch (Exception ex)
            {
        ...
        public Result StoreExcelContentsToTable(int SheetNo, string ExcelSheet, string TableName, DataRow[] ColumnsList)
        {
            try
            {
                ...
                {
                    if (InitializeDataReader(QueryString) == Result.Success)
                    {
                        if (db_Sonic.GetConnection().State == ConnectionState.Closed)
                            db_Sonic.GetConnection().Open();
                        SqlBulkCopy bulk = new SqlBulkCopy(db_Sonic.GetConnection());
                        foreach (DataRow drDetail in ColumnsList)
                        {
                            string colName = drDetail["FieldName"].ToString();
                        ...
                        bulk.DestinationTableName = TableName;
                        bulk.BulkCopyTimeout = 3600000;
                        bulk.WriteToServer(XlsDataReader);
                        ResultsQry += "Message [Result] FROM [" + TableName + "]";
