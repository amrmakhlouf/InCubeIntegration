...
LEFT JOIN Int_Privileges P ON P.PrivilegeID = UP.PrivilegeID AND P.PrivilegeType = UP.PrivilegeType AND UP.PrivilegeType = {1}
LEFT JOIN Int_Field F ON F.FieldID = UP.PrivilegeID AND UP.PrivilegeType = {2}
WHERE UserID = {0} ORDER BY UP.Sequence", UserID, PrivilegeType.MenuAccess.GetHashCode(), PrivilegeType.FieldAccess.GetHashCode()));
