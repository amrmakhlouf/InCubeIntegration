    ...
    public partial class frmDataTransferConfig : Form
    {
        ...
        private void FillDataTransferGroups()
        {
            try
            {
                ...
                Result res = dataTransferMgr.GetTransferGroups(ref dtTransferGroups);
        ...
        private void FillDataTransferTypes()
        {
            try
            {
                ...
                Result res = dataTransferMgr.GetActiveTransferTypes(ref dtTransferTypes);
                if (res == Result.Success)
                {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                frmAddEditDataTransferType frm = new frmAddEditDataTransferType(-1, dataTransferMgr);
                frm.ShowDialog();
                FillDataTransferTypes();
            }
            catch (Exception ex)
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                if (lsvTransferTypes.SelectedItems.Count == 1)
                {
                    ...
                    frmAddEditDataTransferType frm = new frmAddEditDataTransferType(ID, dataTransferMgr);
                    frm.ShowDialog();
                    FillDataTransferTypes();
                }
                else
                {
                    MessageBox.Show("Select data transfer type to edit!!");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (lsvTransferTypes.SelectedItems.Count == 1)
                {
                    ...
                    if (MessageBox.Show("Are you sure you want to delete selected type?", "Confirm", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        {
                            MessageBox.Show("Data transfer type deleted ..");
                            FillDataTransferTypes();
                        }
                        else
                        {
                ...
                {
                    MessageBox.Show("Select data transfer type to delete!!");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            MessageBox.Show(@"Screen not ready yet, manage in DB in table Int_DataTrasferGroups

Group details management is avaialble from below lists after defining group header");
        }
    }
}