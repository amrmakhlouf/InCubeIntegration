...
namespace InCubeIntegration_UI
{
    public partial class frmAddEditDataTransferType : Form
    {
        ...
        public frmAddEditDataTransferType(int ID, DataTransferManager _dataTransferMgr)
        {
            InitializeComponent();
            _id = ID;
        ...
        private void btnAddDestDB_Click(object sender, EventArgs e)
        {
            try
            {
                frmAddEditDatabaseConnection frm = new frmAddEditDatabaseConnection(-1, dataTransferMgr);
                frm.ShowDialog();
        ...
        private void btnEditConnection_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    frmAddEditDatabaseConnection frm = new frmAddEditDatabaseConnection(ID, dataTransferMgr);
                    frm.ShowDialog();
                    FillConnectionsLists();
                }
            }
        ...
        private void FillConnectionsLists()
        {
            try
            {
                ...
                if (dataTransferMgr.GetConnectionsList(ref dtConnections) == Result.Success)
                {
            ...
            try
            {
                using (DataTransferManager transferManager = new DataTransferManager())
                {
                    ...
                    if (result == Result.Success)
                    {
                        ...
                        if (cbSetConstantValues.Checked)
                        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Fill data transfer type name!!");
                    txtName.Focus();
                    return;
                }
                ...
                Result res = dataTransferMgr.SaveTransferType(_id, Name, SelectQuery, SourceDatabaseID, DestinationDatabaseID, DestinationTable, cbIdentity.Checked, (TransferMethod)TransferMethodID, PrimaryKeyColumns, ConstantValues);
                if (res == Result.Success)
                {
                    MessageBox.Show("Data transfer type saved successfully ..");
                    this.Close();
                }
                else
                {
                    MessageBox.Show("Failed in saving Data transfer type!!");
                }
            }
            catch (Exception ex)
            {
