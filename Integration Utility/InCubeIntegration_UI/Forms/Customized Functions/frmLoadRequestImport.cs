...
{
    public partial class frmLoadRequestImport : Form
    {
        ...
        public frmLoadRequestImport()
        {
            InitializeComponent();
            db_LoadImport = new InCubeDatabase();
            db_LoadImport.Open("InCube", "frmLoadRequestImport");
        }
                ...
                if (OpenExcelConnection() == Result.Success)
                {
                    if (CopyExcelContentsToDB() == Result.Success)
                    {
                        if (ProcessData() == Result.Success)
                        {
                            ...
                            lsvContents.AutoResizeColumn(3, ColumnHeaderAutoResizeStyle.ColumnContent);
                        }
                    }
                }
                ...
                if (incubeQry.ExecuteStoredProcedure() == InCubeErrors.Success)
                {
                    ...
                    if (incubeQry.Execute() == InCubeErrors.Success)
                    {
                        ...
                        for (int i = 0; i < dtLoadData.Rows.Count; i++)
                        {
                            ...
                            string ItemKey = dtLoadData.Rows[i]["ItemCode"].ToString() + "-" + dtLoadData.Rows[i]["UOM"].ToString();
                            if (!WHT[Transkey].Details.ContainsKey(ItemKey))
                            {
                                ...
                                TD.Quantity = Convert.ToInt32(dtLoadData.Rows[i]["Quantity"].ToString());
                                ...
                                if (dtLoadData.Rows[i]["ItemID"] != null && dtLoadData.Rows[i]["ItemID"] != DBNull.Value)
                                    TD.ItemID = Convert.ToInt32(dtLoadData.Rows[i]["ItemID"].ToString());
                                ...
                                if (dtLoadData.Rows[i]["PackID"] != null && dtLoadData.Rows[i]["PackID"] != DBNull.Value)
                                    TD.PackID = Convert.ToInt32(dtLoadData.Rows[i]["PackID"].ToString());
                                WHT[Transkey].Details.Add(ItemKey, TD);
                            }
                            else
                            {
                                WHT[Transkey].Details[ItemKey].Quantity += Convert.ToInt32(dtLoadData.Rows[i]["Quantity"].ToString());
                            }
                        }
                    }
                }
        ...
        private void frmLoadRequestImport_Load(object sender, EventArgs e)
        {
...
,1,-1,{5},-1,0)", pair.Value.VehicleID, pair.Value.TransactionID, pair.Value.EmployeeID
...
VALUES ({0},'{1}',1,{2},'1990-01-01',{3},0,'1990/01/01',0,-1,0,{3})"
                            , pair.Value.VehicleID, pair.Value.TransactionID, detail.Value.PackID, detail.Value.Quantity);
