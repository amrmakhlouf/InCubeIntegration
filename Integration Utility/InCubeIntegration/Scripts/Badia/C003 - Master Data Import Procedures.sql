...
inner join ALBADIA_TEST.dbo.Incube_item_master v on e.ItemCATEGORYCODE collate SQL_Latin1_General_CP1256_CI_AS=v.family

delete from ITEMCATEGORYLANGUAGE where ITEMCATEGORYID in (select ITEMCATEGORYID from ITEMCATEGORY where ITEMCATEGORYCODE collate SQL_Latin1_General_CP1256_CI_AS in (select FAMILY from ALBADIA_TEST.dbo.Incube_item_master))
insert into ItemCategoryLanguage
select e.ITEMCATEGORYID,1,v.FAMILY from ItemCategory e inner join ALBADIA_TEST.dbo.Incube_item_master v on e.ITEMCATEGORYCODE collate SQL_Latin1_General_CP1256_CI_AS=v.FAMILY
...
from ALBADIA_TEST.dbo.Incube_item_master v 
...
update t set t.description=v.FAMILY from @TempEmp t inner join ALBADIA_TEST.dbo.Incube_item_master v on t.CODE collate SQL_Latin1_General_CP1256_CI_AS=v.FAMILY
...
from item e 
inner join ALBADIA_TEST.dbo.Incube_item_master v on e.itemcode collate SQL_Latin1_General_CP1256_CI_AS=v.ia_item_code
...
delete from ItemLanguage where ItemID in (select ItemID from Item where ItemCode collate SQL_Latin1_General_CP1256_CI_AS in (select ia_item_code from ALBADIA_TEST.dbo.Incube_item_master))
insert into ItemLanguage
select e.Itemid,1,v.ia_NAME from Item e inner join ALBADIA_TEST.dbo.Incube_item_master v on e.ItemCode collate SQL_Latin1_General_CP1256_CI_AS=v.ia_item_code
...
update t set t.description=v.ia_NAME from @TempEmp t inner join ALBADIA_TEST.dbo.Incube_item_master v on t.CODE collate SQL_Latin1_General_CP1256_CI_AS=v.ia_item_code
...
BEGIN try
BEGIN TRANSACTION T1
update e set e.phone=v.phone, e.inactive=v.status from employee e inner join ALBADIA_TEST.dbo.InCube_Salesman_master v on e.employeecode collate SQL_Latin1_General_CP1256_CI_AS=v.salesman_code where e.employeeid<>0
delete from employeelanguage where employeeid<>0 and employeeid in (select employeeid from employee where employeecode in (select salesman_code from ALBADIA_TEST.dbo.InCube_Salesman_master))
insert into employeelanguage
select e.employeeid,1,v.salesman_name,e.employeecode from employee e inner join ALBADIA_TEST.dbo.InCube_Salesman_master v on e.employeecode=v.salesman_code
...
null,null,null,null,null,null from ALBADIA_TEST.dbo.InCube_Salesman_master v 
where not exists(select employeecode from employee where employeecode=v.Salesman_Code)

update t set t.description=v.Salesman_Name from @TempEmp t inner join ALBADIA_TEST.dbo.InCube_Salesman_master v on t.CODE=v.salesman_code
...
from ALBADIA_TEST.dbo.InCube_Customer_master v 
where not exists(select SimplePeriodWidth from PaymentTerm where SimplePeriodWidth=v.CreditDays)
group by v.CreditDays

update t set t.description=''Every''+cast(v.CreditDays as nvarchar)+''Days'' from @TempEmp t inner join ALBADIA_TEST.dbo.InCube_Customer_master v on cast(t.code as int)=v.CreditDays
...
inner join customer c on c.customerid=ac.customerid inner join ALBADIA_TEST.dbo.InCube_Customer_master v on c.customercode=v.customer_code

update e set  e.onhold=v.status from Customer e inner join ALBADIA_TEST.dbo.InCube_Customer_master v on e.Customercode=v.customer_code

delete from Customerlanguage where Customerid in (select Customerid from Customer where Customercode in (select customer_code from ALBADIA_TEST.dbo.InCube_Customer_master))

insert into Customerlanguage (CustomerID,LanguageID,Description)
select e.Customerid,1,v.customer_name from Customer e inner join ALBADIA_TEST.dbo.InCube_Customer_master v on e.Customercode=v.customer_code
...
update t set t.description=v.customer_Name from @TempEmp t inner join ALBADIA_TEST.dbo.InCube_Customer_master v on t.CODE=v.customer_code
...
select ID+@maxAccCustID,1,v.creditlimit,0,0,1,1,null from @TempEmp t inner join ALBADIA_TEST.dbo.InCube_Customer_master v on t.CODE=v.customer_code 
...
inner join customerOutlet c on c.customerid=ac.customerid and c.outletid=ac.outletid inner join ALBADIA_TEST.dbo.InCube_Customer_master v on c.customercode=v.customer_code

update e set  e.onhold=v.status from CustomerOutlet e inner join ALBADIA_TEST.dbo.InCube_Customer_master v on e.Customercode=v.customer_code
delete from CustomerOutletlanguage where Customerid in (select Customerid from Customer where Customercode in (select customer_code from ALBADIA_TEST.dbo.InCube_Customer_master))
insert into CustomerOutletlanguage (CustomerID,OutletID,LanguageID,Description)
select e.Customerid,1,1,v.customer_name from Customer e inner join ALBADIA_TEST.dbo.InCube_Customer_master v on e.Customercode=v.customer_code

insert into Account (AccountID,AccountTypeID,CreditLimit,Balance,GL,OrganizationID,CurrencyID,ParentAccountID)
select ID+@maxAccCustOutID,1,v.creditlimit,0,0,1,1,ac.accountid from @TempEmp t 
inner join ALBADIA_TEST.dbo.InCube_Customer_master v on t.CODE=v.customer_code 
...
inner join ALBADIA_TEST.dbo.InCube_Salesman_master sm on v.Salesman_name=sm.Salesman_Name
...
BEGIN try
BEGIN TRANSACTION T1
delete from TERRITORYLANGUAGE where TERRITORYID in (select TERRITORYID from TERRITORY where TERRITORYCODE in (select salesman_code from ALBADIA_TEST.dbo.InCube_Salesman_master))
insert into TERRITORYLANGUAGE
select e.TERRITORYID,1,v.salesman_name from TERRITORY e inner join ALBADIA_TEST.dbo.InCube_Salesman_master v on e.TERRITORYCODE=v.salesman_code
...
from ALBADIA_TEST.dbo.InCube_Salesman_master v 
...
delete from ROUTELANGUAGE where ROUTEID in (select ROUTEID from ROUTE where ROUTECODE in (select salesman_code from ALBADIA_TEST.dbo.InCube_Salesman_master))
insert into ROUTELANGUAGE (RouteID,LanguageID,Description)
select e.ROUTEID,1,v.salesman_name from ROUTE e inner join ALBADIA_TEST.dbo.InCube_Salesman_master v on e.ROUTECODE=v.salesman_code
...
BEGIN try
BEGIN TRANSACTION T1
delete from WarehouseLanguage where WarehouseID in (select WarehouseID from Warehouse where WarehouseCODE collate SQL_Latin1_General_CP1256_CI_AS in (select Warehouse_Code from ALBADIA_TEST.dbo.InCube_Warehouses))
insert into WarehouseLanguage
select e.WarehouseID,1,v.Warehouse_Description,null from Warehouse e inner join ALBADIA_TEST.dbo.InCube_Warehouses v on e.WarehouseCODE collate SQL_Latin1_General_CP1256_CI_AS=v.Warehouse_Code
DECLARE @TempEmp TABLE  
(  
    ID INT,  
    CODE VARCHAR(32),
...
from ALBADIA_TEST.dbo.InCube_Warehouses v 
...
update t set t.description =v.Warehouse_Description  collate SQL_Latin1_General_CP1256_CI_AS from @TempEmp t inner join ALBADIA_TEST.dbo.InCube_Warehouses v on t.code collate SQL_Latin1_General_CP1256_CI_AS=v.Warehouse_Code
...
from ALBADIA_TEST.dbo.Incube_Item_price v
inner join item i on v.ia_item_code=i.itemcode collate SQL_Latin1_General_CP1256_CI_AS
...
from ALBADIA_TEST.dbo.Incube_Customer_Item_price v 
...
update t set t.description=v.PR_PC_CODE from @TempEmp t inner join ALBADIA_TEST.dbo.Incube_Customer_Item_price v on t.CODE collate SQL_Latin1_General_CP1256_CI_AS=v.PR_PC_CODE
...
SELECT WAREHOUSEID,WAREHOUSECODE FROM WAREHOUSE WHERE WAREHOUSETYPEID=2 and WarehouseCode collate SQL_Latin1_General_CP1256_CI_AS in (select Warehouse_Code from ALBADIA_TEST.dbo.InCube_Warehouses_stocks)
OPEN VLOOP
...
FROM ALBADIA_TEST.dbo.InCube_Warehouses_stocks CS
...
DECLARE VLOOP CURSOR FOR
SELECT WAREHOUSEID,WAREHOUSECODE FROM WAREHOUSE WHERE WAREHOUSETYPEID=1 and WarehouseCode collate SQL_Latin1_General_CP1256_CI_AS in (select Warehouse_Code from ALBADIA_TEST.dbo.InCube_Warehouses_stocks)
OPEN VLOOP
FETCH NEXT FROM VLOOP INTO @VEHICLEID,@VEHICLECODE
WHILE (@@FETCH_STATUS=0)
BEGIN
