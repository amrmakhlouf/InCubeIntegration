            ...
            try
            {
                switch (CoreGeneral.Common.GeneralConfigurations.SiteSymbol.ToLower())
                {
                    ...
                    case "delmontejo":
                        ...
                        break;
                    case "delmonteus":
                        IntegrationObj = new IntegrationDelMonteUS(CurrentUserID, this);
                        break;
                    case "ufc":
                    ...
                    case "seniora":
                        ...
                        break;
                    case "delmon":
                        IntegrationObj = new IntegrationDelmon(CurrentUserID, this);
                        break;
                    case "freshly":
                        IntegrationObj= new IntegrationFreshly(CurrentUserID, this);
                        break;
                    case "jam":
                        IntegrationObj = new IntegrationJAM(CurrentUserID, this);
                        break;
                    case "viewwater":
                        IntegrationObj = new IntegrationViewWater(CurrentUserID, this);
                        break;
                    case "mezzan":
                        IntegrationObj = new IntegrationMezzan(CurrentUserID, this);
                        break;
                    default:
                ...
                else
                {
                    switch ((IntegrationField)FieldID)
                    {
                        ...
                        case IntegrationField.ExtractTransactionsMapImages_SP:
                            ...
                            break;
                        case IntegrationField.DataWarehouse_SP:
                            IntegrationObj.RunDataWarehouse();
                            break;
                    }
                }
                if (!IntegrationObj.Initialized)
...
,CASE P.PrivilegeType WHEN 1 THEN P.Description WHEN 2 THEN F.FieldName WHEN 3 THEN I.Name WHEN 4 THEN P.Description END Name
...
WHERE E.EmployeeTypeID <> 2 AND E.Inactive = 0 AND E.OrganizationID IN ({0}) AND E.EmployeeID <> 0", CoreGeneral.Common.userPrivileges.Organizations);
        ...
        public int LogActionTriggerBegining(int TaskID, int ActionID, int FieldID, int ImportTypeID)
        {
            int id = -1;
            object ID = null;
            try
            {
                string insertQuery = string.Format(@"INSERT INTO Int_ActionTrigger (SessionID,TaskID,ActionID,FieldID,RunTimeStart,ImportTypeID) VALUES ({0},{1},{2},{3},GETDATE(),{4});
                    SELECT SCOPE_IDENTITY();", CoreGeneral.Common.CurrentSession.SessionID, TaskID, ActionID, FieldID, ImportTypeID);
                incubeQuery = new InCubeQuery(db_exec, insertQuery);
                incubeQuery.ExecuteScalar(ref ID);
                id = int.Parse(ID.ToString());
            }
        ...
        {
            return LogActionTriggerBegining(TaskID, ActionID, FieldID, -1);
        }
        ...
        public DataTable GetAllFieldExecutionSummary(DateTime FromDate, DateTime ToDate, int FieldID)
        {
...
FROM (
SELECT ID,FieldID,DATEDIFF(SS,RunTimeStart,RunTimeEnd) TimeInterval 
FROM Int_ActionTrigger AT
INNER JOIN Int_UserPrivileges UP ON UP.PrivilegeID = AT.FieldID AND UP.PrivilegeType = 2 AND UP.UserID = {3}
WHERE AT.RunTimeEnd IS NOT NULL 
AND AT.RunTimeStart >= '{0}'
AND AT.RunTimeEnd <= '{1}'
{2}
) T
...
, FieldID == -1 ? "" : "AND AT.FieldID = " + FieldID, CoreGeneral.Common.CurrentSession.UserID));
                incubeQuery.Execute();
        ...
        public DataTable GetFieldTriggersList(long FieldID, long MinTriggerID, long MaxTriggerID)
        {
            DataTable dtTriggersList = new DataTable();
            try
            {
                incubeQuery = new InCubeQuery(db_exec, string.Format(@"SELECT AT.ID TriggerID, CASE WHEN AT.TaskID <> -1 THEN 'Scheduled' ELSE 'Manual' END Mode,
CASE WHEN AT.TaskID <> -1 THEN 'Service' ELSE El.Description END [User], FORMAT(AT.RunTimeStart,'yyyy/MM/dd HH:mm:ss') [Start]
, FORMAT(AT.RunTimeEnd,'yyyy/MM/dd HH:mm:ss') [End], dbo.RepresentTime(DATEDIFF(SS,AT.RunTimeStart,AT.RunTimeEnd),1,1,1) [Duration]
, DATEDIFF(SS,AT.RunTimeStart,AT.RunTimeEnd) DurationSeconds
, ISNULL(AT.TotalRows,0) TotalRows
FROM Int_ActionTrigger AT
LEFT JOIN Int_Tasks T ON T.TaskID = AT.TaskID
LEFT JOIN Int_Session S ON S.SessionID = AT.SessionID
LEFT JOIN EmployeeOperator EO ON EO.OperatorID = S.UserID
LEFT JOIN EmployeeLanguage EL ON EL.EmployeeID = EO.EmployeeID AND EL.LanguageID = 1
WHERE AT.ID >= {0} AND ID <= {1} AND AT.FieldID = {2} AND AT.RunTimeEnd IS NOT NULL 
ORDER BY AT.ID
", MinTriggerID, MaxTriggerID, FieldID));
                incubeQuery.Execute();
                dtTriggersList = incubeQuery.GetDataTable();
            }
            catch (Exception ex)
            {
                Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message, ex.StackTrace, LoggingType.Error, LoggingFiles.InCubeLog);
            }
            return dtTriggersList;
        }
        public void Dispose()
        {
            try
