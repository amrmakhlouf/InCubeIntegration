   ...
   public static class Tools
    {
       ...
       /* public static DataTable GetRequestTable<T>(string URL, string body, string JsonAtt, string CertificatePath, string CertificateKey,string Method, WebHeaderCollection webHeader = null)
        {
            T[] x = GetRequest<T>(URL, body, JsonAtt, CertificatePath, CertificateKey, Method,webHeader);
            return ToDataTable<T>(x.ToList<T>());
        }*/
        ...
        {
            return    GetRequestNew<T>(  URL,   body,   JsonAtt,   CertificatePath,   CertificateKey,"","","POST",webHeader);
        }
        ...
        {
            return GetRequestNew<T>(URL, body, JsonAtt, "", "", user, passowrd, Method,webHeader);
        }
        public static T[] GetRequest<T>(string URL, string body, string JsonAtt, out string responseBody, WebHeaderCollection webHeader = null, WebHeaderCollection cookies = null)
        {
            return GetRequestNew<T>(URL, body, JsonAtt, "", "", out responseBody, webHeader, cookies);
        }

        public static T[] GetRequestNew<T>(string URL, string body, string JsonAtt, string CertificatePath, string CertificateKey, out string responseBody, WebHeaderCollection webHeader = null, WebHeaderCollection cookies = null)
        {
            string resp = "";
            try
            {
                ...
                var client = new RestClient(URL);
                {
                    ...
                    if (webHeader != null)
                    {
                        ...
                        {
                            request.AddHeader(webHeader.Keys[i], webHeader.GetValues(i)[0].ToString());
                        }
                    }
                    ...
                    if (cookies != null)
                    {
                        ...
                        {
                            request.AddCookie(cookies.Keys[i], cookies.GetValues(i)[0].ToString());
                        }
                    }
        ...
        public static Dictionary<string, object> GetTokenAndCookies(string url, WebHeaderCollection webHeader)
        {
            ...
            try
            {
                ...
                var csrfTokenHeader = response.Headers?.FirstOrDefault(h => h.Name.Equals("x-csrf-token", StringComparison.OrdinalIgnoreCase));
                string csrfToken = csrfTokenHeader?.Value?.ToString();
                result["token"] = csrfToken;
                ...
                result["cookies"] = cookiesDictionary;
                return result;
            }
            catch (Exception ex)
            {
        ...
        public static T[] GetRequestNew<T>(string URL, string body, string JsonAtt, string CertificatePath, string CertificateKey, string user, string passowrd, string WebMethod, WebHeaderCollection webHeader = null)
        {
            string resp = "";
            try
            {
                ...
                var client = new RestClient(URL);
                {
                    ...
                    if (webHeader != null)
                    {
                        for (int i = 0; i < webHeader.Count; i++)
                        {
                            //request.AddCookie(webHeader.Keys[i], webHeader.GetValues(i)[0].ToString());
                            request.AddHeader(webHeader.Keys[i], webHeader.GetValues(i)[0].ToString());
                        }
                    }
                    if (body.Trim()!="")
