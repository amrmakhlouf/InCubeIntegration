...
WHERE A.INACTIVE=0 AND A.CUSTCLAS not in ('DOUBTFULL-DEBTS','') and A.SALSTERR<>'' and A.SALSTERR<>'JEEMA'
) X Inner JOIN RM00101 Z ON X.BillToId = z.CUSTNMBR

";
            //WriteMessage("get customers correctly");
            ...
            while (err == InCubeErrors.Success)
            {
                ...
                WriteMessage("\r\n");
                WriteMessage("Calling Update Payment Term Function");
                ...
                WriteMessage("\r\n");
                WriteMessage("Update Customer Outlet");
                while (err == InCubeErrors.Success)
                {
                    ...
                    WriteMessage("\r\n");
                    WriteMessage(paymenttermid.ToString());
                    ...
                    else
                    {
                        ...
                        //WriteMessage("Insert " +err.ToString());
                    ...
                    //WriteMessage("update customer outlet language");
        ...
        {
            WriteMessage("\r\n");
            WriteMessage("Start Updating Payment Term" + " " + PYMTRMID);
            InCubeErrors err;
            object field = new object();
...
Where S.PYMTRMID = '" + PYMTRMID + "'";
            ...
            WriteMessage("\r\n");
            WriteMessage("Get Query Result of Payment Term");
            //WriteMessage("get customers correctly");
            ...
            if (DUEDTDS == 0)
                ...
                WriteMessage("\r\n");
                WriteMessage("Finish Update Payment Term");
...
";
        ...
        public override void UpdatePrice()
        {
            ...
            while (err == InCubeErrors.Success)
            {
                ...
                string SelectCustomer = @"select CustomerID,OutletID from CustomerOutlet WHERE CustomerCode='" + CustCode + "' and Barcode='" + CustCode + "'";
...
WHERE RM00101.SALSTERR <> '' and RM00101.INACTIVE=0 AND HOLD=0
";
        ...
        private void UpdateStockForWarehouse(string WarehouseID, DateTime StockDate)
        {
            ...
            string GPStock = @"select HD.GPNo,LN.ITEMNMBR,LN.ATYALLOC,LN.UOFM,HD.DOCDATE,RMS.SALSTERR from JMGPHDRWORK HD
...
and RMS.SALSTERR = '" + WarehouseCode + @"'";
...
WHERE (PackTypeLanguage.LanguageID = 1) AND (ItemLanguage.LanguageID = 1) AND (TransactionDetail.TransactionID = '" + TransactionID.ToString() + "')";
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        Discount = Math.Round(Discount / Quantity, 3, MidpointRounding.AwayFromZero);
                        ...
                        salesLine.UNITPRCE = Math.Round(unitPrice, 2, MidpointRounding.AwayFromZero);
                        TOTAL += (XTNDPRCE);
        ...
        public int SerializeSalesOrderObjectSendPayment(string filename, bool AllSalespersons, string Salesperson, DateTime FromDate, DateTime ToDate)
        {
            ...
            while (err == InCubeErrors.Success)
            {
                ...
                try
                {
                    ...
                    incubeQry = new InCubeQuery(db_vms, "SELECT CP.*, T.TransactionDate FROM CustomerPayment CP INNER JOIN [Transaction] T ON T.TransactionID = CP.TransactionID WHERE CP.CustomerPaymentID = '" + PaymentID + "'");
                    incubeQry.Execute();
                    DataRow[] DetailsPaymentRow = incubeQry.GetDataTable().Select();
                    ...
                    if (PaymentType == "1")// "Cash"
                    {
                        ...
                        decimal TotalValue = Math.Round(Amount, 2, MidpointRounding.AwayFromZero);
                        ...
                        for (int i = 0; i < DetailsPaymentRow.Length; i++)
                        {
                            ...
                            decimal APPTOAMTVALUE = Math.Round(decimal.Parse(Row["AppliedAmount"].ToString()), 2, MidpointRounding.AwayFromZero);
                    ...
                    else if (PaymentType == "4")// "Credit Note"
                    {
                        ...
                        for (int i = 0; i < DetailsPaymentRow.Length; i++)
                        {
                            ...
                            decimal APPTOAMTVALUE = Math.Round(decimal.Parse(Row["AppliedAmount"].ToString()) / 100, 2, MidpointRounding.AwayFromZero);
                            RMApply.APPTOAMT = APPTOAMTVALUE;
                            //RMApply.APPTOAMT = Math.Floor(decimal.Parse(Row["AppliedAmount"].ToString()) * 100) / 100;
                            RMApply.APFRDCTY = 8;
                            RMApply.APTODCTY = 1;
                    ...
                    else if (PaymentType == "3")// "Post Dated Cheque"
                    {
                        ...
                        incubeQry = new InCubeQuery(db_ERP, "SELECT ISNULL(MAX(DEX_ROW_ID),0)+1 FROM PDC00002");
                        object maxID = null;
                        incubeQry.ExecuteScalar(ref maxID);
                        string Query = string.Format(@"
INSERT INTO PDC00002
(DOCNUMBR,pdcDocType,pdcDocStatus,DOCID,DOCDATE,CUSTNMBR,CHEKBKID,CHEKNMBR,CHEKDATE,CHEKAMNT,CURTRXAM
,CURNCYID,BANKID,SLPRSNID,CHKCOMNT,pdcXChangeDocNumber,pdcMarkedToPost,pdcDocStatus1,CBKIDCHK,TIME1
,USERID,pdcMasterDocNumber,TIMESPRT,DEX_ROW_ID)
VALUES ('{0}',1,1,'PDC-RECEIPTS','{1}','{2}','PDC-HHT','{3}','{1}',{4},{4}
,'DHS','{5}','{6}','','',0,0,'',GETDATE()
,'{6}','',0,{7})",
PaymentID, date.ToString("yyyy-MM-dd"), CustomerCode, ChNumber, Amount
, BankID, SalesPersonCode, maxID);
                        incubeQry = new InCubeQuery(db_ERP, Query);
                        ...
                        for (int i = 0; i < DetailsPaymentRow.Length; i++)
                        {
                            ...
                            incubeQry = new InCubeQuery(db_ERP, "SELECT ISNULL(MAX(DEX_ROW_ID),0)+1 FROM PDC00007");
                            maxID = null;
                            incubeQry.ExecuteScalar(ref maxID);

                            Query = string.Format(@"INSERT INTO PDC00007 (DOCNUMBR,APTODCTY,APTODCNM,APPTOAMT,APTODCDT,USERID,pdcDocType,DEX_ROW_ID)
VALUES ('{0}',1,'{1}',{2},'{3}','{4}',1,{5})",
    PaymentID, TransactionID, APPTOAMTVALUE, TransactionDate.ToString("yyyy-MM-dd"), SalesPersonCode, maxID);
                            incubeQry = new InCubeQuery(db_ERP, Query);
                            incubeQry.ExecuteNonQuery();
                        }
                        ...
                        ////string Query = @"INSERT INTO PDC00002 (PDCRVNO, PDCRVDT, PDCCUSTID, PDCDOCNO, PDCDOCDT, PDCBANK, PDCAMOUNT, PDCONHANDDATE) 
