...
{
    public class IntegrationTeleLink : IntegrationBase // Live branch
    {
        ...
        public IntegrationTeleLink(long CurrentUserID, ExecutionManager ExecManager)
            : base(ExecManager)
        {
        ...
        public override void UpdateSerialStock()
        {
            _WarehouseID = Filters.WarehouseID.ToString();
            GetMasterData(IntegrationField.SerialStock_U);
        }
            ...
            try
            {
                ...
                switch (field)
                {
                    ...
                    case IntegrationField.Stock_U:
                        ...
                        break;
                    case IntegrationField.SerialStock_U:
                        res = GetSerialStockTable(ref dtMasterData);
                        StagingTable = "Stg_SerialStock";
                        ProcName = "sp_UpdateSerialStock";
                        break;
...
')", TableName, columns.Substring(0, columns.Length - 1));
        ...
        private Result GetItemTable(ref DataTable DT)
        {
            ...
            {// { \"field\" : \"ISNULL(isAssetItem)\", \"operation\" : \"=\",\"value\" : 0 }
                ...
                DT = Tools.GetRequestTable<TelItem>(CoreGeneral.Common.GeneralConfigurations.WS_URL, body, "rows", CoreGeneral.Common.GeneralConfigurations.SSL_File, CoreGeneral.Common.GeneralConfigurations.SSL_Key);
        ...
        private Result GetWarehousTable(ref DataTable DT)
        {
            ...
            {
                DT = new DataTable();
                string body = "{\"user\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_UserName + "\",\"password\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_Password + "\",\"command\":\"table\",\"table\":\"Warehouse\",\"fields\":[\"code\",\"name\",\"group\" ] " +
        ...
        private Result GetStockTable(ref DataTable DT)
        {
            ...
            {
                DT = new DataTable();
                string body = "{\"user\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_UserName + "\",\"password\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_Password + "\",\"command\":\"report\",\"report\":\"serialBalances\" " +
                       ",\"filters\" : [  ] " +
                   "}";
                DT = Tools.GetRequestTable<ItemSerials>(CoreGeneral.Common.GeneralConfigurations.WS_URL, body, "rows", CoreGeneral.Common.GeneralConfigurations.SSL_File, CoreGeneral.Common.GeneralConfigurations.SSL_Key);
                if (DT != null && DT.Rows.Count > 0)
                {
                    SaveTable(DT, "stg_ItemSerials");
                }
                   
                 body = "{\"user\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_UserName + "\",\"password\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_Password + "\",\"command\":\"report\",\"report\":\"stockBalance\" " +
        ...
        private Result GetSerialStockTable(ref DataTable DT)
        {
            ...
            try
            {
                string WHCode = "";
                if (_WarehouseID != "-1")
                      WHCode = GetFieldValue("Warehouse", "WarehouseCode", db_vms);
                DT = new DataTable();
                string body = "{\"user\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_UserName + "\",\"password\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_Password + "\",\"command\":\"report\",\"report\":\"serialBalances\" ";
               if(WHCode!="")
                    body += ",\"filters\" : [ { \"field\" : \"warehouse_From\",\"value\" : \""+ WHCode + "\" }" +
                        " ,{ \"field\" : \"warehouse_To\",\"value\" : \"" + WHCode + "\" }" +
                        "  ] ";
                body += "}";
        ...
        public override void SendInvoices()
        {
            try
            {
                ...
                string customerid="",outletid="",CustomerCode = "", PackID="",isSerial = "", BlockNumber = "", TransactionID = "", WarehouseCode = "", Currency = "";
...
"\"comment\":\"InCube {11}\",\"manualNum\":\"{13}\"";//,\"receipt\":{12}";
                ...
                string detailsTemp = "\"item\":\"{0}\",\"unit\":\"{1}\",\"price\":\"{2}\",\"quantity\":\"{3}\",\"bonus\":\"{4}\",\"discountPercent\":\"{5}\",\"serial\":\"{6}\" ";
...
t.customerid , t.outletid
                ...
                for (int i = 0; i < dtInvoices.Rows.Count; i++)
                {
                    try
                    {
                        ...
                        string invoiceDetails = string.Format(@"select PackID,
TransactionID,	Quantity,	round(Price+(Price*TaxPer/100),2) price,	round(BasePrice+(BasePrice*TaxPer/100),2) BasePrice	,Discount	,Tax	,ItemCode	,PackName	,DiscPer	,TaxPer,	ItemType ,HasSerialNumber from (
SELECT     
TransactionDetail.PackID,TransactionDetail.TransactionID,
...
group by TransactionDetail.PackID,
...
TransactionDetail.Price 
,TransactionDetail.CustomerID,TransactionDetail.BasePrice, TransactionDetail.OutletID,Pack.HasSerialNumber
...
", TransactionID);
                        ...
                        for (int j = 0; j < dtDetails.Rows.Count; j++)
                        {
                            ...
                            isSerial = dtDetails.Rows[j]["HasSerialNumber"].ToString();
                            ...
                            Serials = "";
                            if (isSerial == "1" || isSerial == "True")
                            {
                               
                                    incubeQuery = new InCubeQuery(db_vms, @"select STUFF((
select ',' + FromSerial from serialtranstracking where TransactionID = '" + TransactionID + "' and Customerid = "+customerid+" and OutletID = "+outletid+" and PackID = " + PackID + "  FOR XML PATH('')),1,1,'' ) as serials ");
                                    incubeQuery.ExecuteScalar(ref Serials);
                            ...
                            allDetails += "{" + string.Format(detailsTemp, ItemCode, UOM, Price, Quantity, bonus, discountPer, Serials.ToString()) + "},";
...
SELECT 2 PType,  CP.CustomerPaymentID,e.EmployeeCode, CP.PaymentDate,  PaidAmount AppliedAmount,0 SecondCurrencyAmount,1 ExchangeRate,c.Description Currency, '' Currency2,o.CustomerCode,cp.VoucherDate,cp.VoucherNumber,b.Code bank, Bl.description branch
									  ...
									  FROM CustomerUnallocatedPayment CP  
											left join CustomerOutlet  o on o.CustomerID=cp.CustomerID and  o.OutletID=cp.OutletID  
                                              ...
                                              Left Join Bank B on CP.BankID=B.BankID  
                                                Left Join BankBranchLanguage Bl on CP.BankID=Bl.BankID and CP.BranchID=Bl.BranchID and Bl.LanguageID=1
                                                WHERE       isnull(CP.voided,0) <>1 AND CP.PaymentTypeID IN (1,2,3)
...
"
        ...
        //                                      ,cp.notes   ,O.RoadNumber CusAccount, O.ShopNumber SupAccount ,O.BlockNumber     FROM CustomerPayment CP left join [transaction] t on cp.TransactionID=t.TransactionID
        //											left join CustomerOutlet  o on o.CustomerID=cp.CustomerID and  o.OutletID=cp.OutletID  
        //												left join CurrencyLanguage c on cp.CurrencyID=c.CurrencyID and c.LanguageID=1
        //												left join CurrencyLanguage c2 on cp.SecondCurrencyID=c2.CurrencyID and c2.LanguageID=1
        //                                                 left join Employee e on cp.EmployeeID=e.EmployeeID
        ...
        public override void SendOrders( )
        {
            try
            {
                ...
                string headerTemp = "\"table\":\"{5}\",\"__TRANSACTION_ID\":\"{0}\",\"__DEVICE_ID\":\"{1}\",\"contact\":\"{2}\",\"salesman\":\"{3}\"," +
"\"docDate\":\"{4}\",\"currency\":\"01\",\"docFormat\":\"Include Tax\",\"warehouse\":\"{6}\"," +
"\"orderDetail\":{7},\"discountPercent\":\"{8}\",\"discountTotal\":\"{9}\",\"totalNet\":\"{10}\"," +
"\"comment\":\"InCube {11}\"";//,\"receipt\":{12}";
                ...
                string invoicesHeader = string.Format(@"SELECT   T.OrderID,   T.OrderDate ,  V.WarehouseCode,e.Employeecode , CO.CustomerCode , t.Discount,t.NetTotal,CO.BlockNumber
                    FROM SalesOrder T 
                    INNER JOIN CustomerOutlet CO ON CO.CustomerID = T.CustomerID AND CO.OutletID = T.OutletID
                    INNER JOIN Customer  C  ON C .CustomerID = T.CustomerID --AND CO.OutletID = T.OutletID
                    INNER JOIN Organization O ON O.OrganizationID = T.OrganizationID
                  ...
                  {2}
                    AND T.OrderStatusID <> 5 /*and T.TransactionID = 'INV-VC11-0071-000040'*/"
                    , Filters.FromDate.ToString("yyyy-MM-dd"), Filters.ToDate.AddDays(1).ToString("yyyy-MM-dd"), salespersonFilter);
                incubeQuery = new InCubeQuery(db_vms, invoicesHeader);
...
SalesOrderDetail.Price* SalesOrderDetail.Tax  Tax, 
Item.ItemCode,  
PackTypeLanguage.Description AS PackName,    
...
", TransactionID);
                        ...
                        string headerData = string.Format(headerTemp, TransactionID.Replace("OINV-", "1").Replace("OID-", "1").Replace("INV-", "1").Replace("RTN-", "2").Replace("-", ""), "", CustomerCode, Salesperson, DateTime.Parse(TransactionDate.ToString()).ToString("dd/MM/yyyy"), SalesDocType, WarehouseCode,
                           allDetails, "0", "0", netTotal, TransactionID);//, "\"\"");
                        string body = "{\"user\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_UserName + "\",\"password\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_Password + "\",\"command\":\"save\",\"record\":{" + headerData + "}}";
                        result[] result1 = Tools.GetRequest<result>(CoreGeneral.Common.GeneralConfigurations.WS_URL, body, "", CoreGeneral.Common.GeneralConfigurations.SSL_File, CoreGeneral.Common.GeneralConfigurations.SSL_Key);
                        if (result1 == null || (result1[0].error != null && result1[0].error.Trim() != ""))
                        {
                        ...
                        else
                        {
                            ...
                            result.Append("ERP No: " + result1[0].rows.code + "\r\n Message:" + (result1[0].error != null ? result1[0].error : "") + "\r\n json:" + body);
                            WriteMessage("Success, ERP No: " + result1[0].rows.code);
                            incubeQuery = new InCubeQuery(db_vms, "UPDATE [SalesOrder] SET Synchronized = 1,Description='"+ result1[0].rows.code+"' WHERE OrderID = '" + TransactionID + "'");
                            incubeQuery.ExecuteNonQuery();
                        }
        ...
        public override void SendTransfers()
        {
            try
            {
                string salespersonFilter = "",  DocType = "" , fromWH = "", toWh = "", employee = "";
                string vendor = "",   TransactionID = "", SalesDocType="";
                DateTime TransactionDate;
                int   processID = 0;
                StringBuilder result = new StringBuilder();
                Result res = Result.UnKnown;

                string headerTemp = "\"table\":\"{5}\",\"__TRANSACTION_ID\":\"{0}\",\"__DEVICE_ID\":\"{1}\",\"contact\":\"{2}\"{3}," +
"\"docDate\":\"{4}\", \"warehouse\":\"{6}\" ,\"issueType\":\"{8}\",\"warehouseOther\":\"{9}\"," +   
"\"orderDetail\":{7}";


                string detailsTemp = "\"item\":\"{0}\",\"unit\":\"{1}\",\"quantity\":\"{2}\" ";
             
                if (Filters.EmployeeID != -1)
                {
                    salespersonFilter = "AND wt.EmployeeID = " + Filters.EmployeeID;
                }
                string invoicesHeader = string.Format(@"select wt.transactionid,wt.transactiondate,
w.Warehousecode WH1,rw.Warehousecode WH2,e.EmployeeCode Employeecode,TransactionOperationID
...
--  wt.WarehouseID not in (select WarehouseID from [Transaction] where   [Transaction].transactiontypeid in(1,2,3,4) AND Voided <> 1 AND Posted = 1 AND Synchronized = 0 ) and
 TransactionOperationID in(1,2,5,6,7) and WarehouseTransactionStatusID in(4,5,6)
...
{2}"
                ...
                {
                    res = Result.Failure;
                    throw (new Exception("Warehouse Transaction header query failed !!"));
                }
                ...
                DataTable dtInvoices = incubeQuery.GetDataTable();
                if (dtInvoices.Rows.Count == 0)
                    WriteMessage("There is no Warehouse Transaction to send ..");
                else
                    SetProgressMax(dtInvoices.Rows.Count);
                ...
                for (int i = 0; i < dtInvoices.Rows.Count; i++)
                {
                    try
                    {
                        ...
                        TransactionID = dtInvoices.Rows[i]["TransactionID"].ToString();
                        ReportProgress("Sending Warehouse Transaction : " + TransactionID);
                        WriteMessage("\r\n" + TransactionID + ": ");
                        Dictionary<int, string> filters = new Dictionary<int, string>();
                        filters.Add(24, TransactionID);
                        processID = execManager.LogIntegrationBegining(TriggerID, -1, filters);
                        ...
                        if (lastRes == Result.Success || lastRes == Result.Duplicate)
                        {
                            ...
                            incubeQuery.ExecuteNonQuery();
                            throw (new Exception("Warehouse Transaction already sent  check table  Int_ExecutionDetails!!"));
                        }



                        TransactionDate = Convert.ToDateTime(dtInvoices.Rows[i]["transactiondate"]);
                        DocType = dtInvoices.Rows[i]["TransactionOperationID"].ToString();
                        employee= dtInvoices.Rows[i]["Employeecode"].ToString();
                        if (DocType == "1" || DocType == "2" || DocType == "6")
                        {
                            fromWH = dtInvoices.Rows[i]["WH2"].ToString();
                            toWh = dtInvoices.Rows[i]["WH1"].ToString();
                         
                        }
                       else
                        {
                            toWh  = dtInvoices.Rows[i]["WH2"].ToString();
                            fromWH = dtInvoices.Rows[i]["WH1"].ToString();
                            
                        }

                        if (DocType == "1" || DocType == "2" || DocType == "5")
                            SalesDocType = "WAREHOUSE";
                        else
                            SalesDocType = "CONTACT";


                        string invoiceDetails = string.Format(@"select item.ItemCode,PackTypeLanguage.Description UOM,sum(wd.Quantity) Quantity from WhTransDetail wd INNER JOIN
Pack ON wd.PackID = Pack.PackID INNER JOIN
Item ON Pack.ItemID = Item.ItemID INNER JOIN 
PackTypeLanguage ON Pack.PackTypeID = PackTypeLanguage.PackTypeID and LanguageID=1
where TransactionID='{0}'
group by item.ItemCode,PackTypeLanguage.Description
", TransactionID);
                        incubeQuery = new InCubeQuery(db_vms, invoiceDetails);
                        if (incubeQuery.Execute() != InCubeErrors.Success)
                        {
                            res = Result.Failure;
                            throw (new Exception("Warehouse transaction details query failed !!"));
                        }
                        ...
                        for (int j = 0; j < dtDetails.Rows.Count; j++)
                        {
                            ...
                            allDetails += "{" + string.Format(detailsTemp, ItemCode, UOM , Quantity ) + "},";
                        ...
                        //,\"salesman\":\"{3}\"
                        string headerData = string.Format(headerTemp, TransactionID.Replace("OINV-", "1").Replace("WT-", "1").Replace("OID-", "1").Replace("INV-", "1").Replace("RTN-", "2").Replace("-", ""), "", vendor, "", DateTime.Parse(TransactionDate.ToString()).ToString("dd/MM/yyyy"), "storeIssueVoucher", fromWH,
                           allDetails , SalesDocType,toWh);//, "\"\"");
                        string body = "{\"user\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_UserName + "\",\"password\":\"" + CoreGeneral.Common.GeneralConfigurations.WS_Password + "\",\"command\":\"save\",\"record\":{" + headerData + "}}";
                        result[] result1 = Tools.GetRequest<result>(CoreGeneral.Common.GeneralConfigurations.WS_URL, body, "", CoreGeneral.Common.GeneralConfigurations.SSL_File, CoreGeneral.Common.GeneralConfigurations.SSL_Key);
                        if (result1 == null || (result1[0].error != null && result1[0].error.Trim() != ""))
                        {
                        ...
                        else
                        {
                            ...
                            result.Append("ERP No: " + result1[0].rows.code + "\r\n Message:" + (result1[0].error != null ? result1[0].error : "") + "\r\n json:" + body);
                            WriteMessage("Success, ERP No: " + result1[0].rows.code);
                            incubeQuery = new InCubeQuery(db_vms, "UPDATE [WarehouseTransaction] SET Synchronized = 1,Notes=isnull(Notes,'') +'" + result1[0].rows.code + "' WHERE TransactionID = '" + TransactionID + "'");
                            incubeQuery.ExecuteNonQuery();
                        }
            ...
            {
                Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message, LoggingType.Error, LoggingFiles.InCubeLog);
                WriteMessage("Fetching Warehouse Transaction failed !!");
            }

        }
    }
    class TelItem : PalItem
    {
