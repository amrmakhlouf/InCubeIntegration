        ...
        private void AddUpdateWarehouse(string warehouseType, string WarehouseCode, string WarehouseName, string ParentCode, string OrganizationID, ref int TOTALUPDATED, ref int TOTALINSERTED)
        {
            ...
            if (warehouseType.Trim().Equals("2"))
            {
                ...
                if (!MainWH.Equals(string.Empty)) // Exist Warehouse --- Update Query
                {
                    ...
                    string OffloadingWH = GetFieldValue("VehicleOffloadingWh", "VehicleID", "vehicleID =" + vehID + " and WarehouseID=" + MainWH + "", db_vms).Trim();
                    if (OffloadingWH.Equals(string.Empty)) // Exist Warehouse --- Update Query
                    {
                        QueryBuilderObject.SetField("VehicleID", WarehouseID);
                        QueryBuilderObject.SetField("WarehouseID", MainWH);
                        err = QueryBuilderObject.InsertQueryString("VehicleOffloadingWh", db_vms);
                    }
                }
            }
...
AND COT.TerritoryID = R.TerritoryID WHERE COT.TerritoryID IS NULL");
        ...
        private void CreateCustomerOutlet(string CategoryCode, string CategoryName, string customerID, string parentAccount, string supervisorID, string GroupID, string GroupID2, string B2B_Inv, string CustType, string CustomerCode, string channelDescription, string Paymentterms, string CustomerDescriptionEnglish, string CustomerAddressEnglish, string CustomerDescriptionArabic, string CustomerAddressArabic, string Phonenumber, string Faxnumber, string OnHold, string Taxable, string HeadOfficeCode, string CreditLimit, string Balance, string Longitude, string latitude, string CustomerBarCode, string email, string divisionID, string inactive, string organizationID, string streetID, string RouteCode, string RouteName, string SalesmanCode, string ClassificationID, string PayerCode, string DivisionCode, string ChannelID, string inactiveOnChannel)
        {
            ...
            string PayerID = GetFieldValue("Payer", "PayerID", "PayerCode = '" + PayerCode + "'", db_vms);
            if (PayerID == string.Empty)
            {
                PayerID = GetFieldValue("Payer", "ISNULL(MAX(PayerID),0) + 1", db_vms);
                QueryBuilderObject.SetField("PayerID", PayerID);
                QueryBuilderObject.SetStringField("PayerCode", PayerCode);
                err = QueryBuilderObject.InsertQueryString("Payer", db_vms);
            }

            string IsPayerAssignedToThisCustomer = GetFieldValue("PayerAssignment", "PayerID", "CustomerID = " + CustomerID + " AND OutletID = " + OutletID + " AND DivisionID = " + divisionID, db_vms).Trim();
            if (IsPayerAssignedToThisCustomer.Equals(string.Empty))
            {
                QueryBuilderObject.SetField("PayerID", PayerID);
                QueryBuilderObject.SetField("CustomerID", CustomerID.ToString());
                QueryBuilderObject.SetField("OutletID", OutletID.ToString());
                QueryBuilderObject.SetField("DivisionID", divisionID.ToString());
                QueryBuilderObject.SetField("CustomerGroupID", "-1");
                QueryBuilderObject.SetField("AllCustomers", "0");
                err = QueryBuilderObject.InsertQueryString("PayerAssignment", db_vms);
            }
            else
            {
                QueryBuilderObject.SetField("PayerID", PayerID);
                err = QueryBuilderObject.UpdateQueryString("PayerAssignment", "CustomerID = " + CustomerID + " AND OutletID = " + OutletID + " AND DivisionID = " + divisionID, db_vms);
            }
        ...
        public override void SendInvoices()
        {
            try
            {
                ...
                if (Filters.EmployeeID != -1)
                    EmpFilter = "AND EmployeeID = " + Filters.EmployeeID;

                string QueryString = string.Format(@"
SELECT TransactionID, CustomerID, OutletID
FROM [Transaction]
WHERE Synchronized = 0 AND ISNULL(Notes,'0') <> 'ERP' AND TransactionTypeID < 5
AND TransactionDate >= '{0}' AND TransactionDate < '{1}'
{2}", Filters.FromDate.ToString("yyyy-MM-dd"), Filters.ToDate.AddDays(1).ToString("yyyy-MM-dd"), EmpFilter);

                incubeQuery = new InCubeQuery(db_vms, QueryString);
                if (incubeQuery.Execute() != InCubeErrors.Success)
                {
                    WriteMessage("Header query failed, check log!!");
                    Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, QueryString, "", LoggingType.Error, LoggingFiles.errorInv);
                    return;
                }
                ...
                for (int i = 0; i < dtInvoices.Rows.Count; i++)
                {
                    ...
                    try
                    {
                        TransactionNumber = dtInvoices.Rows[i]["TransactionID"].ToString();
                        CustomerID = dtInvoices.Rows[i]["CustomerID"].ToString();
                        OutletID = dtInvoices.Rows[i]["OutletID"].ToString();
                        WriteMessage("\r\nTrans No (" + TransactionNumber + "): ");

                        string dtlQryStr = string.Format(@"SELECT CO.CustomerID, CO.OutletID, D.DivisionID, O.OrganizationCode COMPANYCODE, O.OrganizationCode SalesOrganization, ISNULL(D.DivisionCode,'70') DivisionCode,'' DistributionChannel, E.EmployeeCode,
CASE WHEN T.NetTotal = 0 THEN 'ZHF' WHEN T.TransactionTypeID IN (1,3) AND CO.CustomerTypeID = 1 THEN 'ZHS' WHEN T.TransactionTypeID IN (1,3) AND CO.CustomerTypeID = 2 THEN 'ZHE' 
WHEN T.TransactionTypeID IN (2,4) AND TD.PackStatusID IN (0,3) THEN 'ZHR' WHEN T.TransactionTypeID IN (2,4) AND TD.PackStatusID IN (1,2) THEN 'ZVR' ELSE 'XX' END DocumentType,
T.TransactionDate, T.TransactionID TransactionNumber, T.Discount HeaderDiscount, T.LPONumber LPO_Number, CO.CustomerCode ShipToCode, Y.PayerCode, R.TerritoryCode VehicleCode, 
RANK() OVER(ORDER BY I.ItemCode,PTL.Description)*10 LineNumber,
CASE WHEN T.NetTotal = 0 THEN '136' WHEN T.TransactionTypeID IN (1,3) THEN ''
WHEN T.TransactionTypeID IN (2,4) AND TD.PackStatusID IN (0,3) THEN '104' WHEN T.TransactionTypeID IN (2,4) AND TD.PackStatusID IN (1,2) THEN '102' ELSE 'XX' END ItemReasonCode,
RRL.Description ItemReasonCode, I.ItemCode, PTL.Description UOM, SUM(TD.Quantity) Quantity, 0 Quantity2, TD.Price, SUM(TD.Price * TD.Quantity - TD.Discount) TotalLineAmount,
CASE TD.SalesTransactionTypeID WHEN 2 THEN 1 WHEN 3 THEN 1 WHEN 4 THEN 1 ELSE 0 END FOC_Indicator, SUM(TD.Discount) ItemDiscount, CASE T.Voided WHEN 1 THEN 2 ELSE 1 END ActionType,
C.CustomerCode SoldToCode, E.EmployeeID, TD.SalesTransactionTypeID, CASE W.WarehouseTypeID WHEN 1 THEN '77110001' ELSE W.warehouseCode END warehouseCode
FROM 
[Transaction] T 
...
INNER JOIN Employee E ON E.EmployeeID = (CASE WHEN T.RouteID IS NULL THEN (SELECT EmployeeID FROM CustomerLocation WHERE CustomerID = T.CustomerID AND OutletID = T.OutletID) 
WHEN T.RouteID = '' THEN (SELECT EmployeeID FROM CustomerLocation WHERE CustomerID = T.CustomerID AND OutletID = T.OutletID)  ELSE T.EmployeeID  END)
LEFT JOIN Territory R ON R.TerritoryID = (CASE T.RouteID WHEN NULL THEN (SELECT TerritoryID FROM EmployeeTerritory WHERE EmployeeID = E.EmployeeID) ELSE (SELECT TerritoryID FROM Route WHERE RouteID = T.RouteID)  END)
...
INNER JOIN (SELECT CustomerID,OutletID,DivisionID,PayerID,RANK() OVER(PARTITION BY CustomerID, OutletID ORDER BY DivisionID) RNK FROM PayerAssignment) PA 
ON CO.CustomerID = PA.CustomerID AND PA.OutletID = CO.OutletID AND PA.RNK = 1
INNER JOIN Payer Y ON Y.PayerID = PA.PayerID
INNER JOIN Organization O ON T.OrganizationID = O.OrganizationID
LEFT JOIN Division D ON D.DivisionID = ISNULL(T.DivisionID,1)
...
WHERE T.Synchronized = 0 AND ISNULL(T.Notes,'0') <> 'ERP' AND T.TransactionTypeID < 5
AND T.TransactionID = '{0}' AND T.CustomerID = {1} AND T.OutletID = {2}
GROUP BY CO.CustomerID, CO.OutletID, D.DivisionID, O.OrganizationCode, D.DivisionCode, E.EmployeeCode, T.NetTotal, T.TransactionTypeID, CO.CustomerTypeID, TD.PackStatusID,
T.TransactionDate, T.TransactionID, T.Discount, T.LPONumber, CO.CustomerCode, Y.PayerCode, R.TerritoryCode, RRL.Description, I.ItemCode, PTL.Description, TD.Price,
C.CustomerCode, E.EmployeeID, TD.SalesTransactionTypeID, W.WarehouseTypeID, W.warehouseCode, T.Voided
", TransactionNumber, CustomerID, OutletID);
                        incubeQuery = new InCubeQuery(db_vms, dtlQryStr);
                        if (incubeQuery.Execute() != InCubeErrors.Success)
                        {
                            WriteMessage("Details query failed, check log!!");
                            Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, dtlQryStr, "", LoggingType.Error, LoggingFiles.errorInv);
                            continue;
                        }

                        DataTable DetailTable = incubeQuery.GetDataTable();
                        if (DetailTable.Rows.Count == 0)
                        {
                            WriteMessage("No details found");
                            continue;
                        }
                        ...
                        {
                            string existInPool = GetFieldValue("[QNIE_TransactionSendingPool]", "EmployeeID", "TransactionID='" + TransactionNumber + "'", db_vms).Trim();
                        ...
                        {
                            func.Invoke(dest);
                            incubeQuery = new InCubeQuery(db_vms, "INSERT INTO QNIE_TransactionSendingPool VALUES(" + EmployeeID + ",'" + TransactionNumber + "')");
                            err = incubeQuery.Execute();
                        }
                        incubeQuery = new InCubeQuery(db_vms, string.Format("UPDATE [Transaction] SET Synchronized = 1 WHERE TransactionID = '{0}' AND CustomerID = {1} AND OutletID = {2}", TransactionNumber, CustomerID, OutletID));
                        err = incubeQuery.Execute();
        ...
        public override void SendTransfers()
        {
            try
            {
                ...
                if (Filters.EmployeeID != -1)
                {
                    empFilter = "AND WT.RequestedBy = " + Filters.EmployeeID;
                }

                string QueryString = string.Format(@"SELECT WT.TransactionID
FROM WarehouseTransaction WT
LEFT JOIN WH_Sync S ON S.TRANSACTIONID = WT.TransactionID
WHERE WT.TransactionTypeID IN (1,2) AND WarehouseTransactionStatusID IN (4,5) AND S.TRANSACTIONID IS NULL
...
{2}", Filters.FromDate.ToString("yyyy-MM-dd"), Filters.ToDate.AddDays(1).ToString("yyyy-MM-dd"), empFilter);
                ...
                if (incubeQuery.Execute() != InCubeErrors.Success)
                {
                    WriteMessage("Load request header query failed, check log!!");
                    Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, QueryString, "", LoggingType.Error, LoggingFiles.errorTSFR);
                    return;
                }
                DataTable dtLoad = incubeQuery.GetDataTable();
                ClearProgress();
                SetProgressMax(dtLoad.Rows.Count);
                ...
                for (int i = 0; i < dtLoad.Rows.Count; i++)
                {
                    ...
                    try
                    {
                        TransactionID = dtLoad.Rows[i]["TransactionID"].ToString();
                        WriteMessage("\r\nTrans No (" + TransactionID + "): ");

                        string dtlQryStr = string.Format(@"
SELECT 
ISNULL(O.OrganizationCode,'7710') ComapnyCode, WT.TransactionID, W1.WarehouseCode WHFrom, WT.TransactionDate, 
CASE WHEN WT.WarehouseTransactionStatusID = 1 THEN 1 WHEN WT.WarehouseTransactionStatusID >= 4 THEN 5 ELSE 0 END Status,
'' Notes, WT.TransactionTypeID TransactionType, 0 IsLiquidation, WT.TransactionDate DeliveryDate, I.ItemCode, PTL.Description UOM, SUM(WD.Quantity) Qty,
O.OrganizationCode SalesOrganizationCode, RANK()OVER(ORDER BY WD.PackID)LineNumber, ISNULL(D.DivisionCode,'70') DivisionCode, T.TerritoryCode, W2.WarehouseCode LocationTo
FROM WarehouseTransaction WT
INNER JOIN WhTransDetail WD ON WD.TransactionID = WT.TransactionID AND WD.WarehouseID = WT.WarehouseID
INNER JOIN Warehouse W1 ON W1.WarehouseID = WT.RefWarehouseID
INNER JOIN Warehouse W2 ON W2.WarehouseID = WT.WarehouseID
LEFT JOIN Organization O ON O.OrganizationID = WT.OrganizationID
INNER JOIN Pack P ON P.PackID = WD.PackID
INNER JOIN Item I ON I.ItemID = P.ItemID
INNER JOIN PackTypeLanguage PTL ON PTL.PackTypeID = P.PackTypeID AND PTL.LanguageID = 1
LEFT JOIN EmployeeVehicle EV ON EV.VehicleID = W2.WarehouseID
LEFT JOIN EmployeeTerritory ET ON ET.EmployeeID = EV.EMPLOYEEID
LEFT JOIN Territory T ON T.TerritoryID = ET.TerritoryID
INNER JOIN ItemCategory IC ON IC.ItemCategoryID = I.ItemCategoryID
LEFT JOIN Division D ON D.DivisionID = IC.DivisionID
WHERE WT.TransactionID = '{0}'
GROUP BY O.OrganizationCode, WT.TransactionID, W1.WarehouseCode, WT.TransactionDate,WT.WarehouseTransactionStatusID, WT.TransactionTypeID, WT.TransactionDate, I.ItemCode, PTL.Description,
WD.PackID, D.DivisionCode, T.TerritoryCode, W2.WarehouseCode", TransactionID);
                        incubeQuery = new InCubeQuery(db_vms, dtlQryStr);

                        if (incubeQuery.Execute() != InCubeErrors.Success)
                        {
                            WriteMessage("Details query failed, check log!!");
                            Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, dtlQryStr, "", LoggingType.Error, LoggingFiles.errorTSFR);
                            continue;
                        }
...
FROM WarehouseTransaction WT
LEFT JOIN WH_Sync S ON S.TRANSACTIONID = WT.TransactionID
WHERE WT.TransactionTypeID = 3 AND WarehouseTransactionStatusID = 4 AND S.TRANSACTIONID IS NULL
AND WT.TransactionDate >= '{0}' AND WT.TransactionDate < '{1}'
{2}", Filters.FromDate.ToString("yyyy-MM-dd"), Filters.ToDate.AddDays(1).ToString("yyyy-MM-dd"), empFilter);
                ...
                if (incubeQuery.Execute() != InCubeErrors.Success)
                {
                    WriteMessage("Input voucher header query failed, check log!!");
                    Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, QueryString, "", LoggingType.Error, LoggingFiles.errorTSFR);
                    return;
                }
                DataTable dtInput = incubeQuery.GetDataTable();
                ClearProgress();
                SetProgressMax(dtInput.Rows.Count);
                ...
                for (int i = 0; i < dtInput.Rows.Count; i++)
                {
                    ...
                    try
                    {
                        TransactionID = dtInput.Rows[i]["TransactionID"];
                        WriteMessage("\r\nTrans No (" + TransactionID + "): ");

                        string dtlQryStr = string.Format(@"SELECT 
ISNULL(O.OrganizationCode,'7710') ComapnyCode, WT.TransactionID, W1.WarehouseCode WHFrom, WT.TransactionDate, 
CASE WHEN WT.WarehouseTransactionStatusID = 1 THEN 1 WHEN WT.WarehouseTransactionStatusID >= 4 THEN 5 ELSE 0 END Status,
'' Notes, WT.TransactionTypeID TransactionType, 0 IsLiquidation, WT.TransactionDate DeliveryDate, I.ItemCode, PTL.Description UOM, SUM(WD.Quantity) Qty, 
O.OrganizationCode SalesOrganizationCode, RANK() OVER(ORDER BY WD.PackID) LineNumber, ISNULL(D.DivisionCode,'70') DivisionCode
--, WT.LPONumber, V.VendorCode 
, '' LPONumber, '200022' VendorCode
FROM 
WarehouseTransaction WT 
INNER JOIN WhTransDetail WD ON WD.TransactionID = WT.TransactionID AND WD.WarehouseID = WT.WarehouseID
INNER JOIN Warehouse W1 ON W1.WarehouseID = WT.WarehouseID
LEFT JOIN Organization O ON O.OrganizationID = WT.OrganizationID
INNER JOIN Pack P ON P.PackID = WD.PackID
INNER JOIN Item I ON I.ItemID = P.ItemID
INNER JOIN PackTypeLanguage PTL ON PTL.PackTypeID = P.PackTypeID AND PTL.LanguageID = 1
INNER JOIN ItemCategory IC ON IC.ItemCategoryID = I.ItemCategoryID
LEFT JOIN Division D ON D.DivisionID = IC.DivisionID
--LEFT JOIN Vendor V ON V.VendorID = WT.VendorID
WHERE WT.TransactionID = '{0}'
GROUP BY O.OrganizationCode, WT.TransactionID, W1.WarehouseCode, WT.TransactionDate, WT.WarehouseTransactionStatusID, WT.TransactionTypeID, WT.TransactionDate, I.ItemCode, PTL.Description,
WD.PackID, D.DivisionCode--, WT.LPONumber, V.VendorCode", TransactionID);
                        incubeQuery = new InCubeQuery(db_vms, dtlQryStr);
                        if (incubeQuery.Execute() != InCubeErrors.Success)
                        {
                            WriteMessage("Details query failed, check log!!");
        ...
        private void GetSequenceNumberFormat1(string maxTransactionOrderID, ref string charactersPart, ref string numericPart)
        {
            try
            {
        ...
        private string GetMaxCreditNoteID1(string EmployeeID)
        {
            try
            {
                ...
                {
                    //GetSequenceNumberFormat1(MaxCreditNote, ref charactersPart, ref numericPart);
...
";
        ...
        private void InvoiceInterface1()
        {
            try
            {
...
";
        ...
        private decimal BaseUOMQuantity1(string ItemID, string PackID, decimal Quantity, decimal PackQty)
        {
        ...
        private void UpdateInvoice1()
        {
            ...
            {
                DataTable DT = new DataTable();
                //DataTable DT = GetInvoiceTable1(ref Tran_Counter);
        ...
        private void UpdatePromotion1()
        {
            ...
            {
                DataTable DT = new DataTable();
                //DataTable DT = GetPromotionTable1();
...
WHERE CO.GroupID={0}
 ", GroupIDExist);
        ...
        private void UpdateStock1()
        {
            try
            {
                //UpdateStockForWarehouse1(Filters.StockDate);
                //UpdateStock2();
            }
            catch
            {
        ...
        private void UpdateStock2()
        {
            ...
            //DataTable DT = GetMainWHStockTable1();
            DataTable DT = new DataTable();
            ClearProgress();
            SetProgressMax(DT.Rows.Count);
...
Item.ItemCode ='{0}' order by Pack.Quantity desc", ItemCode.Trim()));
        ...
        private void UpdateVehicleStock1(bool UpdateAll, string WarehouseID, bool CaseChecked, DateTime StockDate)
        {
            //UpdateStockForWarehouse1(StockDate);
        }
        private void UpdateStockForWarehouse1(DateTime stockDate)
        {
            try
            {
                int TOTALUPDATED = 0;
                ...
                //DTBL = GetApprovedLoad1(ref transactionsList);
...
GO
        ...
        COMMIT TRANSACTION T1  
                            ...
                            */
                            ...
                            if (ItemID.Equals(string.Empty))
                            {
                                ExtendItem1(ItemCode, DivisionCode, OrganizationID, ref tran);
                            ...
                            if (existDetail.Equals(string.Empty))
                            {
                                ...
                                if (err != InCubeErrors.Success) { throw new Exception("Error"); }
                                //err = UpdateQNIEVehicleStock1(ref tran, ItemID, vehicleID, expiry, Batch, PackID, StkQty);
                                if (err != InCubeErrors.Success) { throw new Exception("Error"); }
                            }
                            else
                            {
        ...
        private InCubeErrors UpdateQNIEVehicleStock1(ref InCubeTransaction tran, string ItemID, string vehicleID, string Expirydate, string Batch, string PackID, string Quantity)
        {
        ...
        private void AccounteRecursiveFunction1(int accountID, decimal balance, bool IsIncrease, string PayerCode)
        {
            try
            {
        ...
        private DataTable BalanceBase1()
        {
            DataTable DT = new DataTable();
            try
            {
...
where t. transactiontypeid in (1,3,5,6) and t.voided<>1 and t.remainingamount>0 ";//and t.customerid=8855
        ...
        private void CreateCreditNote1(string GRV, string EmployeeID)
        {
            string CreditNoteID = "";
            //string CreditNoteID = GetMaxCreditNoteID1(EmployeeID);
            int _rowaffected = 0;
...
FROM [Transaction] Where TransactionID = '" + GRV + "' AND TransactionTypeID = 2";
        ...
        private DataTable Exceptions1(string field)
        {
            ///// Passing a table from SAP to .Net //////
        ...
        private InCubeErrors GetCarrefourCustomerInfo1(string CarrefourCustomerCode, ref string CustomerID, ref string OutletID)
        {
        ...
        private InCubeErrors GetCarrefourEmpoyeeInfo1(ref string EmployeeID, ref string OrderID)
        {
            InCubeErrors result = InCubeErrors.Error;
            ...
            try
            {
                ...
                if (incubeQuery.Execute() == InCubeErrors.Success)
                {
                    ...
                    if (dtData != null && dtData.Rows.Count == 1)
                    {
                        ...
                        //GetSequenceNumberFormat1(maxOrderID, ref charPart, ref numericPart);
        ...
        private DataTable GetInvoiceTable1(ref int counter)
        {
            DataTable DT = new DataTable();
            try
            {
        ...
        private DataTable OLDGetInvoiceTableOLD1(string Dist_Channel, ref int counter)
        {
            DataTable DT = new DataTable();
            try
            {
        ...
        private DataTable GetApprovedLoad1(ref List<string> TransactionsList)
        {
            DataTable DT = new DataTable();
            try
            {
        ...
        private DataTable GetMainWHStockTable1()
        {
            RfcDestination prd = RfcDestinationManager.GetDestination(GetServerName);
            RfcRepository repo = prd.Repository;
        ...
        private DataTable GetOrderStatusTable1()
        {
            ///// Passing a table from SAP to .Net //////
            RfcDestination prd = RfcDestinationManager.GetDestination(SendServerName);
            RfcRepository repo = prd.Repository;
        ...
        private DataTable GetPromotionTable1()
        {
            RfcDestination prd = RfcDestinationManager.GetDestination(GetServerName);
            RfcRepository repo = prd.Repository;
        ...
        private DataTable GetStockTable1()
        {
            ///// Passing a table from SAP to .Net //////
            RfcDestination prd = RfcDestinationManager.GetDestination(SendServerName);
            RfcRepository repo = prd.Repository;
        ...
        private DataTable GetUOMTable1()
        {
            DataTable DT = new DataTable();
            try
            {
        ...
        private void ReadConfigurations1()
        {
            try
            {
        ...
        private void ResetAccountBalance1(string query)
        {
            object field = null;
        ...
        private void ExtendItem1(string ItemCode, string DivisionCode, string OrganizationID, ref InCubeTransaction tran)
        {
            try
            {
...
";
        ...
        private void SendDownPaymentReciepts1(bool AllSalespersons, string Salesperson, DateTime FromDate, DateTime ToDate)
        {
            try
            {
                WriteMessage("\r\n" + "Sending Receipts");
...
WHERE (PackTypeLanguage.LanguageID = 1) AND (ItemLanguage.LanguageID = 1) AND (TransactionDetail.TransactionID = '" + TransactionID.ToString() + "')";
        ...
        private void SendReturnOrders1(bool AllSalespersons, string Salesperson, DateTime FromDate, DateTime ToDate)
        {
...
AND (TransactionDetail.TransactionID = '" + TransactionID.ToString() + @"')";
        ...
        private void SendRoadNetLocations1()
        {
            //            InCubeQuery qry ;
