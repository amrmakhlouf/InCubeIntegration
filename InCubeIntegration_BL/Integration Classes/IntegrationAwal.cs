...
on a.AccountID=b.AccountID";
        ...
        public override void UpdateCustomer()
        {
            try
            {
                ...
                object obj2 = new object();
                string cmdText = "SELECT   CustomerCode,CustomerBarcode,ArabicDescription,EnglishDescription,Phone,Fax,Email,MainAddressEnglish,MainAddressArabic,Taxable,Groups,IsCredit,Creditlimit,Balance,PaymentTerms,OnHold,MasterCustomerCode,isKeyAccount,TaxNumber,InActive,IsCreditOnly,NVL(ISCASHONLY,0) ISCASHONLY FROM Customer";
                DefaultOrganization();
                DataTable dataTable = new DataTable();
                GetOracleDataTable(cmdText, ref dataTable);
                ClearProgress();
                ...
                for (int i = 0; i < dataTable.Rows.Count; i++)
                {
                    ...
                    string CashOnly = dataTable.Rows[i]["ISCASHONLY"].ToString(); 
                    string ConfigGroup = "";
                    if (isCredit == "0")
                    {
                        CustomerTypeID = 1;
        ...
        public override void UpdateKPI()
        {
            try
            {
                GetOracleMasterData(IntegrationField.Target_U);
            }
            catch (Exception ex)
            {
                Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message, LoggingType.Error, LoggingFiles.InCubeLog);
