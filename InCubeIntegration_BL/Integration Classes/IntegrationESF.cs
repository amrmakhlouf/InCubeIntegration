...
, T.LPONumber
,T.NetTotal HeaderNet, T.GrossTotal HeaderGross, T.Discount HeaderDiscount, T.Tax HeaderTax, T.DivisionID, T.ExciseTax
FROM [Transaction] T 
...
AND (SOD.OrderID = '" + TransID.ToString() + "') AND (SOD.CustomerID = '" + CustomerID.ToString() + "')";
        ...
        public Result SendTransactionWithTax(string filename, bool AllSalespersons, string Salesperson, DateTime FromDate, DateTime ToDate, IntegrationField Field, bool SendTax)
        {
            ...
            try
            {
                ...
                decimal TOTAL = 0, DiscountTotal = 0, TaxTotal = 0, HeaderGross = 0, HeaderDiscount = 0, HeaderTax = 0, HeaderNet = 0, Width = 0, OriginalDiscount = 0, MiscDiscount = 0, LineMisc = 0, HeaderExcise = 0;
                ...
                for (int m = 0; m < dtHeader.Rows.Count; m++)
                {
                    try
                    {
                        ...
                        HeaderExcise = decimal.Parse(dtHeader.Rows[m]["ExciseTax"].ToString());
                        ...
                        if (Math.Abs(HeaderTax + HeaderExcise - salesHdr.TAXAMNT) > AllowedVariance)
                        {
