...
namespace InCubeIntegration_BL
{
    public class IntegrationBRF : IntegrationBase
    {
        ...
        //        SAP_WS.Url = "https://integrationqas.brf-corp.com:51001/XISOAPAdapter/MessageServlet?channel=:BS_PALM_05:CC_PALM_SOAP_SND&amp;version=3.0&amp;Sender.Service=CC_PALM_SOAP_SND&amp;Interface=ITF_O_S_EnvRecArqPalm%5EBS_PALM_05";
        //        SAP_WS.UseDefaultCredentials = false;
        //        SAP_WS.Credentials = new NetworkCredential("SRV_PILEGADOS", "P3RD1G@0");
        ...
        
        {
            if (UserID != 999)
            {
                ...
                if (!CoreGeneral.Common.IsTesting)
                {
                    ...
                    switch (CoreGeneral.Common.GeneralConfigurations.SecurityProtocol.ToLower())
                    {
                        case "ssl3":
...
LEFT JOIN ConfigurationOrganization CO ON CO.OrganizationID = O.OrganizationID AND CO.KeyName = '{0}'", Config.ToString()));
        ...
        private string SetDate(DateTime Date, int TotalLength, DateFormat dateFormat)
        {
            string dateStr = "0".PadLeft(TotalLength);
            try
            {
                switch (dateFormat)
                {
        ...
        private string SetString(string input, int TotalLength, PaddingOption padding, char PaddingChar)
        {
            return SetString(input, TotalLength, padding, PaddingChar, TrimmingOption.Right);
        }
        private string SetString(string input, int TotalLength, PaddingOption padding, char PaddingChar, TrimmingOption trim)
        {
            string output = "".PadLeft(TotalLength, PaddingChar);
            try
            {
                if (input.Length > TotalLength)
                {
                    if (trim == TrimmingOption.Right)
                        input = input.Substring(0, TotalLength);
                    else
                        input = input.Substring(input.Length - TotalLength, TotalLength);
                }
                    
                if (padding == PaddingOption.Left)
                    output = input.PadLeft(TotalLength, PaddingChar);
                else
                    output = input.PadRight(TotalLength, PaddingChar);
            }
            catch (Exception ex)
            {
                Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message, ex.StackTrace, LoggingType.Error, LoggingFiles.InCubeLog);
...
ORDER BY E.Email, SO.OrderID"
                ...
                for (int k = 0; k < dtOrders.Rows.Count; k++)
                {
                    ...
                    try
                    {
                        ...
                        record = new DTP_EnvRecArqPalm_ReqT_Dados_IN();
                        record.Dados = "N0";
                        record.Dados += SetString(Email, 4, PaddingOption.Right, ' ');
                        record.Dados += OrderID.Substring(OrderID.ToString().Length - 5);//Code
                        record.Dados += SetString(CustomerCode, 12, PaddingOption.Left, '0');//customer code
                        record.Dados += SetDate(DeliveryDate, 6, DateFormat.DDMMYY);//delivery date
                        record.Dados += SetDate(OrderDate, 4, DateFormat.DDMM);//month and day
                        record.Dados += SetDecimal(cda.ToString(), 6, 2, false);//CDA
...
AND PBH.CustomerID = {1} AND PBH.OutletID = {2}", OrderID, CustomerID, OutletID));
                        ...
                        for (int j = 0; j < count; j++)
                        {
                            ...
                            record.Dados += SetString(CustomerCode, 12, PaddingOption.Left, '0');//customer code 7 12
                            record.Dados += (j + 1).ToString().PadLeft(4, '0');//salesTxRow[8].ToString().PadLeft(4, '0');//sequance 19 4
                            record.Dados += SetString(salesTxRow[0].ToString(), 18, PaddingOption.Left, '0');//sequance 23 18
                            record.Dados += SetString(salesOrderType, 4, PaddingOption.Left, ' ');//empty 41 4
                            record.Dados += SetDecimal(price.ToString(), 5, ReadConfiguration(Configuration.NumberOfDigits), false);
                            record.Dados += SetDecimal(qty.ToString(), 8, 3, false);//quantity
                            record.Dados += string.Empty.PadLeft(3);// PaymentTerm.ToString().PadLeft(3, '0');//PaymentTerm
                            record.Dados += SetString(salesTxRow[1].ToString(), 3, PaddingOption.Right, ' ');//UOM
                            record.Dados += SetDecimal(itemCDA.ToString(), 6, 2, false);//itemCDA
                            record.Dados += SetDecimal(totalCDA.ToString(), 6, 2, false);//totalCDA
                            record.Dados += "                000000";//Extra
                            ...
                            record.Dados += SetString(PromoID, 10, PaddingOption.Left, ' ');//Extra
                        ...
                        record.Dados += SetString(CustomerCode, 12, PaddingOption.Left, '0');//customer code
                        record.Dados += SetString(LPO, 15, PaddingOption.Right, ' ');//orderID
                        record.Dados += SetString(CustomerCode, 12, PaddingOption.Left, '0');//customer code
                        record.Dados += "N";
                        record.Dados += "N";
                        ...
                        record.Dados += OrderID.ToString().Substring(OrderID.ToString().Length - 5);//Code
                        record.Dados += SetString(CustomerCode, 12, PaddingOption.Left, '0');//customer code
                        record.Dados += SetString((LPO + "-" + Note), 40, PaddingOption.Right, ' ');
...
ORDER BY E.Email, SO.OrderID", Filters.FromDate.Date.ToString("yyyy/MM/dd"), Filters.ToDate.Date.AddDays(1).ToString("yyyy/MM/dd"), OrganizationID, EmpFilter);
                ...
                for (int i = 0; i < dtOrders.Rows.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        record.Dados = "F3";
                        record.Dados += SetString(Email, 4, PaddingOption.Right, ' ');
                        record.Dados += SetString(CustRefNo, 12, PaddingOption.Right, ' ');
                        record.Dados += "ZCRF";
                        record.Dados += SetString(CustomerCode, 10, PaddingOption.Left, '0');
                        record.Dados += SetDate(OrderDate, 6, DateFormat.DDMMYY);
                        record.Dados += SetString(ReturnReasonCode, 3, PaddingOption.Right, ' ');
                        record.Dados += SetString(OrderID, 35, PaddingOption.Right, ' ');
                        record.Dados += SetString(InvoiceRefNo, 16, PaddingOption.Right, ' ');
                        record.Dados += "*";
                        records.Add(record);
...
, OrderID, CustomerID, OutletID, ReadConfiguration(Configuration.NumberOfDigits));
                        ...
                        for (int j = 0; j < dtDetails.Rows.Count; j++)
                        {
                            ...
                            record.Dados = "F4";
                            record.Dados += SetString(Email, 4, PaddingOption.Right, ' ');
                            record.Dados += SetString(CustRefNo, 35, PaddingOption.Right, ' ');
                            record.Dados += ((j + 1) * 100).ToString().PadLeft(6, '0');
                            record.Dados += SetString(ItemCode, 18, PaddingOption.Left, '0');
                            ...
                            record.Dados += SetString(UOM, 3, PaddingOption.Right, ' ');
                            record.Dados += "*";
                            records.Add(record);
                        }
					...
					", Code);
                        ...
                        while (err == InCubeErrors.Success)
                        {
                            try
                            {
                                ...
                                record = new DTP_EnvRecArqPalm_ReqT_Dados_IN();
                                record.Dados = "";
                                record.Dados += SetString(_SalesmanID, 10, PaddingOption.Left, ' ');//SP_GL_IND 13,1
                                record.Dados += SetString(_Currency, 3, PaddingOption.Left, ' ');
                                string Value = ((int)_Value).ToString();
                                record.Dados += SetDecimal(Value, 10, 2, false);
                                record.Dados += SetDate(_DepositeDate, 8, DateFormat.YYYYMMDD);//PAY_TYPE 32,1
                                if (ReadConfiguration(Configuration.SendBagNumberAndDeviceLocation) == 1)
                                {
                                    record.Dados += SetString(bagNumber, 20, PaddingOption.Left, ' ');
                                    record.Dados += SetString(deviceLocation, 80, PaddingOption.Left, ' ');
                                }
...
ORDER BY EmployeeID"
                ...
                for (int j = 0; j < dtPayments.Rows.Count; j++)
                {
                    ...
                    try
                    {
                        ...
                        record.Dados = "";
                        record.Dados += SetString(PaymentID, 10, PaddingOption.Right, ' ', TrimmingOption.Left);//Pay_ID 0,10
                        ...
                        record.Dados += SetDate(date, 8, DateFormat.DDMMYYYY);//TRAN_DATE 14,8
                        record.Dados += SetString(TransactionID, 10, PaddingOption.Right, ' ', TrimmingOption.Left);//TRAN_ID 22,10
                        record.Dados += SetString(Pay_Type, 1, PaddingOption.Right, ' ');//PAY_TYPE 32,1
                        record.Dados += SetString(SalesPersonCode, 10, PaddingOption.Left, ' ');//EMP_CODE 33,10
                        record.Dados += SetString(OutletCode, 10, PaddingOption.Left, '0');//CUST_CODE 43,10
                        record.Dados += SetString(OutletCode, 10, PaddingOption.Left, '0');//OUTLET_CODE 53,10
                        record.Dados += "0";//DB_CR_IND 63,1
                        record.Dados += SetDecimal(Amount.ToString(), 13, ReadConfiguration(Configuration.NumberOfDigits), true);
                        record.Dados += string.Empty.PadLeft(3);//SALES_GRP 77,3
                        record.Dados += "01";//DIVISION 80,2
                        record.Dados += "10";//DIS_CHL 82,2
                        record.Dados += SetString(note, 10, PaddingOption.Right, ' ');//CR_NO 84,10
                        record.Dados += string.Empty.PadLeft(18);//ALLOC_NMBR 94,18
                        record.Dados += SetString(note2, 16, PaddingOption.Right, ' ');//REF_DOC_NO 112,16
                        record.Dados += SetString(TransactionID, 10, PaddingOption.Right, ' ', TrimmingOption.Left);//INV_REF 128,10
                        record.Dados += string.Empty.PadLeft(4);//SALES_OFFICE 138,4
                        record.Dados += SetString(Down_Type, 2, PaddingOption.Right, ' ');//DOWN_TYPE 142,2
                        record.Dados += SetString(Email, 4, PaddingOption.Right, ' ');//MAIL_BOX 144,4
                        record.Dados += SetString(VOC_NO, 30, PaddingOption.Right, ' ');//no desc 148,30
                        record.Dados += SetString(BankName, 60, PaddingOption.Right, ' ');//BANK 178,60
                        record.Dados += SetString(BranchName, 40, PaddingOption.Right, ' ');//BRANCH 238,40
                        record.Dados += SetString(note2, 50, PaddingOption.Right, ' ');//REC_DATE 278,50
                        record.Dados += SetString(RV, 13, PaddingOption.Right, ' ');//CR_VAL 328,13
                        temp = vouchDate.Year.ToString().PadLeft(4, '0') + vouchDate.Month.ToString().PadLeft(2, '0') + vouchDate.Day.ToString().PadLeft(2, '0');
                        record.Dados += SetDate(vouchDate, 8, DateFormat.YYYYMMDD);
                        record.Dados += SetString(InvItemNumber, 3, PaddingOption.Left, '0', TrimmingOption.Left);
                        record.Dados += "*";
                        records.Add(record);
            ...
            try
            {
                for (int i = 1; i < resp.T_Dados_OUT.Length - 1; i++)
                {
                    try
                    {
                        ...
                        decimal packQty = GetDecimalValue(record, 103, 8, 3);
                        decimal weight = GetDecimalValue(record, 91, 6, 3);
                ...
                try
                {
                    ...
                    switch (docType)
                    {
                        ...
                        case "002": //Order Details (Table = Item Status)
                            ...
                            price = GetDecimalValue(record, 49, 5, ReadConfiguration(Configuration.NumberOfDigits));
                            orderedQty = GetDecimalValue(record, 54, 8, 3);
                            actualQty = GetDecimalValue(record, 62, 8, 3);
                            ...
                            QueryBuilderObject.SetStringField("ItemCode", itemCode);
                            QueryBuilderObject.SetField("Price", price.ToString());
                            QueryBuilderObject.SetField("OrderedQty", orderedQty.ToString());
                            QueryBuilderObject.SetField("ActualQty", actualQty.ToString());
                            err = QueryBuilderObject.InsertQueryString("OrderStatusDetails", db_ERP);
                            break;
                    }
        ...
        public void FillStock(string Email, int WarehouseID)
        {
            for (int i = 1; i < resp.T_Dados_OUT.Length - 1; i++)
            {
                try
                {
                    ...
                    decimal Quantity = GetDecimalValue(record, 31, 7, 0);
                    ...
                    QueryBuilderObject.SetField("Quantity", Quantity.ToString());
        ...
        public void FillInvoices(string email, int EmployeeID)
        {
            try
            {
                for (int i = 1; i < resp.T_Dados_OUT.Length - 1; i++)
                {
                    ...
                    decimal TotalValueAvailable = GetDecimalValue(record, 12, 10, ReadConfiguration(Configuration.NumberOfDigits));
                    string SAPBillingNumber = GetStringField(record, 22, 10);
                    decimal AppliedAmount = GetDecimalValue(record, 32, 10, ReadConfiguration(Configuration.NumberOfDigits));
                    ...
                    QueryBuilderObject.SetField("[LineNo]", (i + 1).ToString());
                    QueryBuilderObject.SetStringField("CustomerCode", CustomerCode);
                    QueryBuilderObject.SetField("TotalValueAvailable", TotalValueAvailable.ToString());
                    QueryBuilderObject.SetStringField("SAPBillingNumber", SAPBillingNumber);
                    QueryBuilderObject.SetField("AppliedAmount", AppliedAmount.ToString());
                ...
                try
                {
                    ...
                    for (int i = 1; i < resp.T_Dados_OUT.Length - 1; i++)
                    {
                        ...
                        if (Taxable != "0" && Taxable != "1")
                            Taxable = "0";
                        decimal CreditLimit = GetDecimalValue(record, 188, 15, ReadConfiguration(Configuration.NumberOfDigits));
                        string CustomerGroup = "";
                        string PriceGroup = "";
                        string Segment = "";
                        string ChannelCode = "";
                        ...
                        if (HeadOfficeCode == string.Empty)
                        {
                            ...
                            {
                                QueryBuilderObject.SetField("CreditLimit", CreditLimit.ToString());
...
update CustomerOutlet set gpslatitude = '0' where gpslatitude is null");
        ...
        private void CreateCustomerOutlet(string CustomerCode, string CustomerGroupID, string PaymentTermDesc, string CustomerDescriptionEnglish, string CustomerAddressEnglish, string CustomerDescriptionArabic, string CustomerAddressArabic, string Phonenumber, string Faxnumber, string OnHold, string Taxable, string HeadOfficeCode, decimal CreditLimit, string Balance, string Longitude, string latitude, string CustomerBarCode, string email, string StreetID, string Contact, string ContactMobile, ref string _custID, ref string _outtletID, string EGroupID, string PriceGroupID, string SegmentID, string HirGroupID, string TerritoryID)
        {
            int CustomerID;
            InCubeErrors err;
