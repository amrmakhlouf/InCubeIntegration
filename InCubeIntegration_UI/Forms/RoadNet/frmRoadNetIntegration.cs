    ...
    public partial class frmRoadNetIntegration : Form
    {
                ...
                foreach (KeyValuePair<Menus, string> MenuPair in CoreGeneral.Common.userPrivileges.MenusAccess)
                {
                    switch (MenuPair.Key)
                    {
                        ...
                        case Menus.Route_Region:
                            ...
                            break;
                        case Menus.StandardInstructions:
                            AddButton(Properties.Resources.instructions, MenuPair.Value, index++).MouseClick += new MouseEventHandler(ModifyStandardInstructions);
                            break;
                        case Menus.PRN_Config:
                            AddButton(Properties.Resources.PRN_Config, MenuPair.Value, index++).MouseClick += new MouseEventHandler(ConfigurePRN);
                            break;
                        case Menus.Locations_Export:
                            AddButton(Properties.Resources.Locations_Export, MenuPair.Value, index++).MouseClick += new MouseEventHandler(LocationsExport);
                            break;
                        case Menus.JorneyPlan_Import:
                            AddButton(Properties.Resources.JourneyPlan_Import, MenuPair.Value, index++).MouseClick += new MouseEventHandler(JourneyPlanImport);
                            break;
                    }
                }
            }
        ...
        private PictureBox AddButton(Bitmap image, string Description, int Index)
        {
            ...
            try
            {
                ...
                tableLayoutPanel1.Controls.Add(pic, 2 * ((Index - 1) % MenusPerRow), 2 * ((Index - 1) / MenusPerRow));
            }
            catch (Exception ex)
            {
                Logger.WriteLog(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message, LoggingType.Error, LoggingFiles.UIErrors);
        ...
        private void ConfigurePRN(object sender, EventArgs e)
        {
            try
            {
                frmPRNConfig frm = new frmPRNConfig();
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void LocationsExport(object sender, EventArgs e)
        {
            try
            {
                frmLocationsExport frm = new frmLocationsExport();
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void JourneyPlanImport(object sender, EventArgs e)
        {
            try
            {
                frmJourneyPlanImport frm = new frmJourneyPlanImport();
                if (!frm.RN_Conn_Opened)
                {
                    MessageBox.Show("Failure in establishing connection to RoadNet DB");
                }
        ...
        private void ModifyStandardInstructions(object sender, EventArgs e)
        {
            try
            {
                frmRoadNetStandardInstructions frm = new frmRoadNetStandardInstructions();
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
