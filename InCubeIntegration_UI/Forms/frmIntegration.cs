            ...
            try
            {
                if (btnRunSpecialActions.Text == "Run")
                {
                    ...
                    for (int i = 0; i < lsvSpecialActions.CheckedItems.Count; i++)
                    {
                        ...
                        if (field == IntegrationField.DataTransfer_SP || field == IntegrationField.DatabaseBackup_SP || field == IntegrationField.FilesJobs_SP || field == IntegrationField.DataWarehouse_SP)
                        {
                            ...
                            else if (filterValues.ContainsKey(BuiltInFilters.DataWarehouseCheckList.GetHashCode()))
                                SpecialActionFilters.SetValue(BuiltInFilters.DataWarehouseCheckList, filterValues[BuiltInFilters.DataWarehouseCheckList.GetHashCode()]);
                        }
