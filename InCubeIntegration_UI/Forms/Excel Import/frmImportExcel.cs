...
namespace InCubeIntegration_UI
{
    public partial class frmImportExcel : Form
    {
        ...
        private void cmbDataType_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (cmbDataType.SelectedIndex >= 0 && cmbDataType.ValueMember != "")
                {
                    ...
                    DropCreate = int.Parse(dtTypes.Select("ImportTypeID = " + selectedImportTypeID)[0]["DropCreateStagingTables"].ToString());
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                em.LogActionTrigger(selectedImportTypeID);
                if (em.TriggerID == -1)
                {
                    MessageBox.Show("Error in initialization, try again!");
                    return;
                }
                ...
                foreach (DataRow dr in selectedSheets)
                {
                    ...
                    DataRow[] selectedColumns = dtSheetsColumns.Select(string.Format("ImportTypeID = {0} AND SheetNo = {1}", selectedImportTypeID, SheetNo));
                    em.DumpDataToStagingTable(SheetNo, ExcelSheet, StagingTable, DropCreate, selectedColumns);
                }
